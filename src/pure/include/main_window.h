...
void playLoseSound() {
    AudioDevice = SDL_OpenAudioDevice(NULL, 0, &lose_soundeffect_spec, NULL, 0);
    SDL_QueueAudio(AudioDevice, lose_soundeffect_buffer, lose_soundeffect_length);
    SDL_PauseAudioDevice(AudioDevice, 0);
}

void playWinSound() {
    AudioDevice = SDL_OpenAudioDevice(NULL, 0, &win_soundeffect_spec, NULL, 0);
    SDL_QueueAudio(AudioDevice, win_soundeffect_buffer, win_soundeffect_length);
    SDL_PauseAudioDevice(AudioDevice, 0);
}
...
void renderStuff() {
    renderHands();
    renderNewSprite(help_menu, 353, 109, 325, 450);
    switch (win_status) {
        case -1: break;
        case 0:
        ...
        while (SDL_PollEvent(&event)) {
            switch (event.type) {
                case SDL_QUIT:
                    ...
                    SDL_DestroyTexture(computer_text);
                    SDL_DestroyTexture(you_text);
                    SDL_DestroyTexture(win_msg);
                    SDL_DestroyTexture(lose_msg);
                    SDL_DestroyTexture(tie_msg);
...
int loadMainWindow() {
    window = SDL_CreateWindow("Rocks, Papers, Scissors!", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1000, 600, SDL_WINDOW_SHOWN);
